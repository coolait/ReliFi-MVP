...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!day || !hour) {
    res.status(400).json({ error: 'Day and hour parameters are required' });
    return;
  }

  const dayData = allDaysData[day.toString().toLowerCase()];
  
  if (!dayData || !dayData[hour.toString()]) {
    res.status(404).json({ error: 'No recommendations found for this time slot' });
    return;
  }
  ...
  res.json({
    day: day,
    hour: hour,
    recommendations: dayData[hour.toString()]
  });
}
